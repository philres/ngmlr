...
bool inline isInversion(float const nm) {
	float const minIdentity = 0.45f;
	float const maxIdentity = 0.65f;
...
bool AlignmentBuffer::alignInversion(Interval interval, Interval leftOfInv,
	...
	/**********************************************/
	inversionLength += (readSeqLen - align.lastPosition.readPosition);
//	inversionLength += (readSeqLen - ((int*) (align.ExtendedData))[3]);
	...
	/**********************************************/
	int secondQStart = align.firstPosition.readPosition;
//	inversionLength += secondQStart;
	inversionLength += secondQStart;
	if (inv.isReverse) {
		inv.onReadStop += (secondQStart * 1.2);
	} else {
		inv.onReadStart -= (secondQStart * 1.2);
	}
	inv.onRefStop += (secondQStart * 1.5);
	if (pacbioDebug) {
		Log.Message("2. pos in read start: %d", secondQStart);
	}
	delete[] readSeq;
	readSeq = 0;
	if (align.ExtendedData != 0) {
...
void AlignmentBuffer::alignSingleOrMultipleIntervals(MappedRead * read,
	...
	if ((inversionDetected = inversionDetection(align, interval,
		...
		inversionAligned = alignInversion(interval, leftOfInv, inv, rightOfInv,
				read, tmpAling, alignIndex, tmp, mq);
		if (!inversionAligned) {
			SequenceLocation loc;
...
void AlignmentBuffer::reconcileRead(ReadGroup * group) {
	...
	//Interval * sortByRefPos = new Interval[read->Calculated];

	//memcpy(sortByRefPos, read->Alignments, sizeof(Align) * read->Calculated);



//	std::sort(sortByRefPos, sortByRefPos + read->Calculated,
//					sortIntervalsInSegment);
	...
	for (int i = 0; i < read->Calculated; ++i) {
		...
		if (stdoutPrintMappedSegments) {
			fprintf(stdout, "%s\t%d\t%d\t%d\t%d\t%d\t%f\n", read->name, i,
					read->length, read->Alignments[i].QStart,
					read->Alignments[i].QEnd,
...
//		int onRead = (isReverse) ? (int) (group->readNumber) - j : j;
		...
		treeIntervals.push_back(
				IntervalTree::Interval<int>(onRead * 512, onRead * 512 + 512,
						part->mappingQlty));
