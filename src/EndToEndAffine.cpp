...
int EndToEndAffine::SingleAlign(int const mode, int const corridor,
	...
	convertToCIGAR(mode, gapsText, gapsPattern, result, len);
	result.Score = score;
	return 1;
}
...
int EndToEndAffine::BatchAlign(int const mode, int const batchSize,
	...
	for (int i = 0; i < batchSize; ++i) {
		...
		convertToCIGAR(mode, gapsText, gapsPattern, results[i], len);
	}
	return batchSize;
}

void EndToEndAffine::convertToCIGAR(int const mode, TGaps gapsText, TGaps gapsPattern,
